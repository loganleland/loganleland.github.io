...
| KBASE_IOCTL_CS_QUEUE_REGISTER | Register a GPU command queue with the base back-end | 36 | _IOW | struct kbase_ioctl_cs_queue_register {<br>  __u64 buffer_gpu_addr;<br>  __u32 buffer_size;<br>  __u8 priority;<br>  __u8 padding[3];<br>}; |
| KBASE_IOCTL_CS_QUEUE_KICK | Kick the GPU command queue group scheduler to notify that a queue has been updated | 37 | _IOW | struct kbase_ioctl_cs_queue_kick {<br>  __u64 buffer_gpu_addr;<br>}; |
...
| KBASE_IOCTL_CS_QUEUE_BIND | Bind a GPU command queue to a group | 39 | _IOW | union kbase_ioctl_cs_queue_bind {<br>  struct {<br>    __u64 buffer_gpu_addr;<br>    __u8 group_handle;<br>    __u8 csi_index;<br>    __u8 padding[6];<br>  } in;<br>  struct {<br>    __u64 mmap_handle;<br>  } out;<br>}; |
| KBASE_IOCTL_CS_QUEUE_REGISTER_EX | Register a GPU command queue with the base back-end in extended format, involving trace buffer configuration | 40 | _IOW | struct kbase_ioctl_cs_queue_register_ex {<br>  __u64 buffer_gpu_addr;<br>  __u32 buffer_size;<br>  __u8 priority;<br>  __u8 padding[3];<br>  __u64 ex_offset_var_addr;<br>  __u64 ex_buffer_base;<br>  __u32 ex_buffer_size;<br>  __u8 ex_event_size;<br>  __u8 ex_event_state;<br>  __u8 ex_padding[2];<br>}; |
| KBASE_IOCTL_CS_QUEUE_TERMINATE | Terminate a GPU command queue| 41 | _IOW | struct kbase_ioctl_cs_queue_terminate {<br>  __u64 buffer_gpu_addr;<br>}; |
| KBASE_IOCTL_CS_QUEUE_GROUP_CREATE_1_6 | Create a GPU command queue group | 42 | _IOWR | union kbase_ioctl_cs_queue_group_create_1_6 {<br>  struct {<br>    __u64 tiler_mask;<br>    __u64 fragment_mask;<br>    __u64 compute_mask;<br>    __u8 cs_min;<br>    __u8 priority;<br>    __u8 tiler_max;<br>    __u8 fragment_max;<br>    __u8 compute_max;<br>    __u8 padding[3];<br>  } in;<br>  struct {<br>    __u8 group_handle;<br>    __u8 padding[3];<br>    __u32 group_uid;<br>  } out;<br>}; |
| KBASE_IOCTL_CS_QUEUE_GROUP_TERMINATE |Terminate a GPU command queue group | 43 | _IOW | struct kbase_ioctl_cs_queue_group_term {<br>  __u8 group_handle;<br>  __u8 padding[7];<br>};|
| KBASE_IOCTL_CS_EVENT_SIGNAL | | 44 | _IO | |
| KBASE_IOCTL_KCPU_QUEUE_CREATE  | Create a KCPU command queue | 45 | _IOR | struct kbase_ioctl_kcpu_queue_new {<br>  base_kcpu_queue_id id;<br>  __u8 padding[7];<br>}; |
| KBASE_IOCTL_KCPU_QUEUE_DELETE  | Destroy a KCPU command queue | 46 | _IOW | struct kbase_ioctl_kcpu_queue_delete {<br>  base_kcpu_queue_id id;<br>  __u8 padding[7];<br>}; |
| KBASE_IOCTL_KCPU_QUEUE_ENQUEUE  | Destroy a KCPU command queue | 47 | _IOW | struct kbase_ioctl_kcpu_queue_enqueue {<br>  __u64 addr;<br>  __u32 nr_commands;<br>  base_kcpu_queue_id id;<br>  __u8 padding[3];<br>}; |
| KBASE_IOCTL_CS_TILER_HEAP_INIT  | Initialize chunked tiler memory heap | 48 | _IOWR | union kbase_ioctl_cs_tiler_heap_init {<br>  struct {<br>    __u32 chunk_size;<br>    __u32 initial_chunks;<br>    __u32 max_chunks;<br>    __u16 target_in_flight;<br>    __u8 group_id;<br>    __u8 padding;<br>    __u64 buf_desc_va;<br>  } in;<br>  struct {<br>    __u64 gpu_heap_va;<br>    __u64 first_chunk_va;<br>  } out;<br>}; |
| KBASE_IOCTL_CS_TILER_HEAP_INIT_1_13  |Initialize chunked tiler memory heap, earlier version upto 1.13| 48 | _IOWR | union kbase_ioctl_cs_tiler_heap_init_1_13 {<br>  struct {<br>    __u32 chunk_size;<br>    __u32 initial_chunks;<br>    __u32 max_chunks;<br>    __u16 target_in_flight;<br>    __u8 group_id;<br>    __u8 padding;<br>  } in;<br>  struct {<br>    __u64 gpu_heap_va;<br>    __u64 first_chunk_va;<br>  } out;<br>}; |
| KBASE_IOCTL_CS_TILER_HEAP_TERM  | Terminate a chunked tiler heap instance | 49 | _IOW | struct kbase_ioctl_cs_tiler_heap_term {<br>  __u64 gpu_heap_va;<br>}; |
...
| KBASE_IOCTL_CS_GET_GLB_IFACE | Request the global control block of CSF interface capabilities | 51 | _IOWR | union kbase_ioctl_cs_get_glb_iface {<br>  struct {<br>    __u32 max_group_num;<br>    __u32 max_total_stream_num;<br>    __u64 groups_ptr;<br>    __u64 streams_ptr;<br>  } in;<br>  struct {<br>    __u32 glb_version;<br>    __u32 features;<br>    __u32 group_num;<br>    __u32 prfcnt_size;<br>    __u32 total_stream_num;<br>    __u32 instr_features;<br>  } out;<br>}; |
| KBASE_IOCTL_VERSION_CHECK | Check version compatibility between kernel and userspace | 52 | _IOWR | struct kbase_ioctl_version_check {<br>  __u16 major;<br>  __u16 minor;<br>}; |
| KBASE_IOCTL_CS_CPU_QUEUE_DUMP | Dump buffer containing cpu queue information | 53 | _IOW | struct kbase_ioctl_cs_cpu_queue_info {<br>  __u64 buffer;<br>  __u64 size;<br>}; |
...
| KBASE_IOCTL_CS_QUEUE_GROUP_CREATE | Create a GPU command queue group | 58 | _IOWR | union kbase_ioctl_cs_queue_group_create {<br>  struct {<br>    __u64 tiler_mask;<br>    __u64 fragment_mask;<br>    __u64 compute_mask;<br>    __u8 cs_min;<br>    __u8 priority;<br>    __u8 tiler_max;<br>    __u8 fragment_max;<br>    __u8 compute_max;<br>    __u8 csi_handlers;<br>    __u8 padding[2];<br>    // @in.reserved: Reserved<br>    __u64 reserved;<br>  } in;<br>  struct {<br>    __u8 group_handle;<br>    __u8 padding[3];<br>    __u32 group_uid;<br>  } out;<br>}; |
| KBASE_IOCTL_MEM_ALLOC_EX | Allocate memory on the GPU | 59 | _IOWR | union kbase_ioctl_mem_alloc_ex {<br>  struct {<br>    __u64 va_pages;<br>    __u64 commit_pages;<br>    __u64 extension;<br>    __u64 flags;<br>    __u64 fixed_address;<br>    __u64 extra[3];<br>  } in;<br>  struct {<br>    __u64 flags;<br>    __u64 gpu_va;<br>  } out;<br>}; |
| KBASE_IOCTL_READ_USER_PAGE | Read a register of USER page | 60 | _IOWR | union kbase_ioctl_read_user_page {<br>  struct {<br>    __u32 offset;<br>    __u32 padding;<br>  } in;<br>  struct {<br>  __u32 val_lo;<br>    __u32 val_hi;<br>  } out;<br>}; |
